...
module.exports = async function handler(req, res) {
  try {
    ...
    const { normalizeGame, validId } = await import('../shared/game-config.mjs');
    ...
    if (req.method === 'GET' && action === 'participants' && id) {
      ...
      const participants = Object.entries(players).map(([id, p]) => ({ id, firstName: p.firstName, lastName: p.lastName, email: p.email, phone: p.phone, gamesPlayed: p.gamesPlayed || 0, highScore: p.highScore ?? legacyScores[id]?.highScore ?? 0, consentMarketing: !!p.consentMarketing, consentGame: !!p.consentGame, consentTimestamp: p.consentTimestamp || p.createdAt, createdAt: p.createdAt }));
    ...
    if (req.method === 'POST' && action === 'save') {
      ...
      let config;
      try { config = normalizeGame(input); } catch (e) { throw new HttpError(400, e.message); }
      ...
      return send(res, 200, { game: { ...game, id: gameId } });
    }
